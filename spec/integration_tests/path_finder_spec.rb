...
describe PathFinder, type: :feature do
  ...
  def read_inputs_from_file
    edges_file_path = File.expand_path(
      '../data/constant_difficulty/test_edge_inputs.json', __dir__
    )
    vertices_file_path = File.expand_path(
      '../data/constant_difficulty/test_vertex_inputs.json', __dir__
    )
  ...
  def generate_inputs(num_vertices)
    TestInputGenerator::TestInputGenerator.execute(
      complete_graph: true,
  ...
  describe 'doing sanity checks on outputs with small input graphs' do
    ...
    it 'the returned vertex list has the same length as the number'\
       'of input vertices' do
      expect(result[:vertices].length).to eq(num_vertices + 1)
    end

    it 'the returned vertex list includes every vertex id'\
       'in the input' do
      vertex_ids = vertex_inputs.map { |el| el[:id] }
      expect(result[:vertices].uniq.sort).to eq(vertex_ids.sort)
    end

    it 'returns a list of edge ids, all of which are included'\
       'in the edges input' do
    ...
    it 'returned edge list contains num_vertices edges' do
      expect(result[:edges].length).to eq(num_vertices)
    end

    it 'path length equals the sum of the cost of traversals of'\
       'the returned edge list' do
      ...
      avg_path_length = (expected_path_length + result[:path_length]) / 2.0
      percent_diff = diff / avg_path_length * 100
    ...
    context 'when include_path_length_vs_iteration is set to true' do
      ...
      it 'path length output has same length as \'num_iterations\''\
         'config variable' do
        expect(result[:iteration_path_lengths].length).to eq(
          PathFinder.config.num_iterations
        )
      end
    end
  end
  ...
  describe 'checking against exact solutions for small complete graphs' do
    ...
    it 'on average, is within 10\% of exact solution' do
      aco_path_lengths = []
      exact_solutions = []

      (0..num_tests - 1).each do
        vertex_inputs, edge_inputs = TestInputGenerator::TestInputGenerator.execute(
          complete_graph: true,
          num_vertices: num_vertices,
          write_to_file: false
        )
  ...
  describe 'checking that path lengths found for large complete graphs'\
           'decay during iteration process' do
    let(:num_vertices) { 50 }
    let(:generated_inputs) do
      TestInputGenerator::TestInputGenerator.execute(
        complete_graph: true,
    ...
    it 'path length decays with successive iterations' do
  ...
  describe 'testing incomplete graphs' do
    ...
    describe 'testing that failures when running incomplete graphs are rare' do
      ...
      it 'doesn\'t raise an error with repeated tests' do
        expect do
          (0..num_tests - 1).each do |i|
    ...
    describe 'testing that an error is raised when there is a failure to'\
             'compute a path' do
      let(:vertex_inputs) do
        [
          { id: 1, x_pos: 5.4, y_pos: 3.1 },
          { id: 2, x_pos: -5.1, y_pos: -9.1 },
          { id: 3, x_pos: -19.1, y_pos: 10.0 },
          { id: 4, x_pos: -4.8, y_pos: -3.9 }
        ]
      end
      let(:edge_inputs) do
        [
          { id: 1, start_vertex_id: 1, end_vertex_id: 2, cost_of_traversal: 8.1 },
          { id: 2, start_vertex_id: 2, end_vertex_id: 1, cost_of_traversal: 1.8 },
          { id: 3, start_vertex_id: 3, end_vertex_id: 4, cost_of_traversal: 4.1 },
          { id: 4, start_vertex_id: 4, end_vertex_id: 3, cost_of_traversal: 9.7 }
        ]
      end

      it 'raises a AntColonyTsp::PathNotFound error' do
